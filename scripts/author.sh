...
DEFAULT_TARGET_REPO="https://github.com/iamvikshan/mina-docs"
...
SIGNING_KEY_PATH="$HOME/.ssh/mina-docs-id_ed25519_signing"
...
if [[ -f "$SIGNING_KEY_PATH" && -f "$SIGNING_KEY_PUB" ]]; then
  ...
  #   - Anyone with read access to ~/.ssh/mina-docs-id_ed25519_signing can use it
...
if [[ "$HAS_SIGNING_SCOPE" != "true" ]]; then
  ...
  if [[ -z "$NORMALIZED_SIGNING_KEY" ]]; then
    ...
    if remote_signing_key_exists "$NORMALIZED_SIGNING_KEY"; then
      ...
      ADD_EXIT_CODE=0
      ADD_OUTPUT=$(gh api -X POST /user/ssh_signing_keys -f key="$NORMALIZED_SIGNING_KEY" -f title="mina-docs-$GIT_USER signing key" 2>&1) || ADD_EXIT_CODE=$?
